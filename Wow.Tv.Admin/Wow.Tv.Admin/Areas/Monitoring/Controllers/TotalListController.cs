...
{
    public class TotalListController : BaseController
    {
        [WowTvAdminAuthorize(IsRead = true)]
        public ActionResult Index(TotalListCondition condition)
        {
            ...
            ListModel<TotalListResult> resultData = new Wow.Tv.Admin.MemberMonitoringService.MemberMonitoringServiceClient().TotalList(condition);
            List<int> userNumberList = new List<int>();
            //List<MemberInfoSimple> userNumberList = new List<MemberInfoSimple>();
            int actionSeq = 0;
            foreach (TotalListResult item in resultData.ListData)
            {
                actionSeq++;
                item.ActionSeq = condition.CurrentIndex + actionSeq;

                string actionType = GetActionType(item.ActionCode);
                if (item.IsActionLog == true)
                {
                    item.ActionDescription = item.AdminId + "가 " + item.MenuName + "에서 " + actionType;
                }
                else
                {
                    item.ActionDescription = item.AdminId + "가 " + item.MenuName + " " + actionType;
                }
                
                if (item.MenuSeq == 401 && item.IsActionLog == true) // 회원정보
                {
        ...
        private string GetActionType(string actionCode)
        {
            ...
            switch (actionCode)
            {
                ...
                case "NO_AUTH_MENU": actionType = "메뉴 접근 시도"; break;
                case "NO_AUTH_IP": actionType = "메뉴 외부 아이피로 접근 시도"; break;
            }
            return actionType;
        }
    }
