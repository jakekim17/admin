$(document).ready(function() {
    ...
    $("#btnDelete").click(function() {
        ...
        $.each($("#tbodyList").find("input[type=\"checkbox\"]"), function (index, item) {
            if ($(item).prop("checked") == true) {
                checkItems.push($(item).val());
            }
        });
...
var NoticeIndex = {
    ...
    GoEdit: function (seq) {
        $("#seq").val(seq);
        $("#frmSearch").attr("method", "POST");
        $("#frmSearch").attr("action", "/ProgramBoard/Notice/Edit?menuSeq=" + $('#CurrentMenuSeq').val() + "&ProgramCode=" + $("#hidProgramCode").val());
        $("#frmSearch").submit();
    ...
    GoAdd: function (currentIndex) {
        $("#currentIndex").val(currentIndex);
        $("#frmSearch").attr("method", "POST");
        $("#frmSearch").attr("action", "/ProgramBoard/Notice/Add?menuSeq=" + $('#CurrentMenuSeq').val() + "&ProgramCode=" + $("#hidProgramCode").val());
        $("#frmSearch").submit();
    ...
    RemoveAll: function (items) {
        $.ajax({
            type: "POST",
            url: "/ProgramBoard/Notice/RemoveAll?menuSeq=" + $('#CurrentMenuSeq').val() + "&ProgramCode=" + $("#hidProgramCode").val(),
            data: { "items": items },
