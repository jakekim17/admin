...
{
    public class AccLogController : BaseController
    {
        [WowTvAdminAuthorize(IsRead = true)]
        public ActionResult Index(AccLogCondition condition)
        {
            ...
            ListModel<AccLogResult> resultData = new Wow.Tv.Admin.MemberMonitoringService.MemberMonitoringServiceClient().AccLog(condition);
            List<int> userNumberList = new List<int>();
            //List<MemberInfoSimple> userNumberList = new List<MemberInfoSimple>();
            int actionSeq = 0;
            foreach (AccLogResult item in resultData.ListData)
            {
                actionSeq++;
