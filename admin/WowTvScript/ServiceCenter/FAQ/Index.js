$(document).ready(function() {
    ...
    $("#btnAdd").click(function() {
        FAQIndex.GoAdd($("#currentIndex").val(), $("#mseq").val() );
...
var FAQIndex = {
    ...
    GoDetail: function (seq) {
        $("#seq").val(seq);
        $("#frmSearch").attr("method", "POST");
        $("#frmSearch").attr("action", "/ServiceCenter/FAQ/Detail");
        $("#frmSearch").submit();
    ...
    GoAdd: function (currentIndex,mseq) {
        $("#currentIndex").val(currentIndex);
        $("#frmSearch").attr("method", "POST");
        $("#frmSearch").attr("action", "/ServiceCenter/FAQ/Add");
        $("#frmSearch").submit();
    ...
    RemoveAll: function (items) {
        $.ajax({
            type: "POST",
            url: "/ServiceCenter/FAQ/RemoveAll",
            data: { "items": items },
            success: function (data) {
                if (data.IsSuccess) {
                    confirm(CommonMsg.DeleteAfterMsg);
                    FAQIndex.GetList(currentIndex);
                }
                else {
                    alert(data.Msg);
